...
#2. try to find out who is having a min balance (we can write this in many ways)
...
#3. try to find out who is having a mazxmim balance 
...
#7. try to find out a record where education is primarty and person is jobless
select * from bank_details where education='primary' and job='unkown';
...
#-------------------------------------
DELIMITER &&
create procedure avg_bal_job_admin()
BEGIN
select avg(balance) from bank_details where job='admin.';
END &&

call  avg_bal_job_admin()
#----------------------------------------

DELIMITER &&
create procedure avg_bal_job_role1(IN jobname varchar(30))
BEGIN
	select avg(balance) from bank_details where job=jobname;
END &&    

call avg_bal_job_role1('admin.');
call avg_bal_job_role1('retired');
call avg_bal_job_role1('unknown')
#-----------------------------------------------
...
call sel_edu_job1('tertiary', 'unknown');
